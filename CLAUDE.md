...
| Public routing | Cloudflare tunnel `knomi-ai` (ID `fb2cb466-3f4f-46f8-8a0c-2b45c549bbe4`) → `http://nomii-frontend-staging:80` on shared docker network. **Container name still `nomii-frontend-staging`** (Phase 6 staging rename deferred — tunnel origin is hardcoded in Cloudflare dashboard and needs Austin to update it there before the compose-side container_name flip). |
