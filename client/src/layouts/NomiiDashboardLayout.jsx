...
const SidebarContent = ({ nomiiTenant, nomiiUser, badges, handleSignOut, subscription, usage }) => {
  const planInfo = subscription ? (PLAN_LABELS[subscription.plan] || PLAN_LABELS.free) : null;
  return (
  <div className="flex flex-col h-full">
    {/* Brand */}
    <div className="px-5 pt-7 pb-5">
      <img src={nomiiLogo} alt="Nomii AI" className="h-8 block mx-auto" />
    </div>
      ...
      <div className="mx-4 mb-4 rounded-xl px-4 py-3" style={{ background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.12)" }}>
        <div className="flex items-center gap-2.5 mb-2">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0" style={{ background: "rgba(201,168,76,0.15)", color: "#C9A84C" }}>
            {nomiiTenant.name?.[0]?.toUpperCase() || "K"}
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-[13px] font-semibold text-white/90 truncate">{nomiiTenant.name}</p>
            {nomiiTenant.agent_name && (
              <p className="text-[11px] text-white/30 truncate">{nomiiTenant.agent_name}</p>
            )}
          </div>
          {planInfo && (
            <span
      ...
      {nomiiUser && (
        <div className="flex items-center gap-3 mb-3">
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0" style={{ background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.6)" }}>
            {(nomiiUser.firstName?.[0] || "").toUpperCase()}{(nomiiUser.lastName?.[0] || "").toUpperCase()}
          </div>
          <div className="min-w-0">
            <p className="text-[12px] text-white/70 font-medium truncate">
              {nomiiUser.firstName} {nomiiUser.lastName}
            </p>
            <p className="text-[11px] text-white/25 truncate">{nomiiUser.email}</p>
          </div>
        </div>
      )}
      <button
...
const NomiiDashboardLayout = () => {
  const { nomiiUser, nomiiTenant, setNomiiUser, subscription } = useNomiiAuth();
  ...
  const sidebarProps = { nomiiTenant, nomiiUser, badges, handleSignOut, subscription, usage: usageData };
...
export default NomiiDashboardLayout;
