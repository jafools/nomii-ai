...
OUT="${BACKUP_DIR}/nomii_ai_${TS}.sql.gz"
...
# Retain last N days
find "$BACKUP_DIR" -name "nomii_ai_*.sql.gz" -mtime +${RETAIN_DAYS} -delete

SIZE=$(du -sh "$OUT" | cut -f1)
COUNT=$(ls "$BACKUP_DIR"/nomii_ai_*.sql.gz 2>/dev/null | wc -l | tr -d ' ')
