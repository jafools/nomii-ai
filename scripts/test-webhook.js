...
try {
  tenantId = execSync(
    `docker exec shenmay-db psql --U shenmay -d shenmay_ai -t -c "SELECT tenant_id FROM tenant_admins WHERE email = '${EMAIL}' LIMIT 1;"`,
    { encoding: 'utf8' }
  ).trim();
} catch (e) {
    ...
    if (res.statusCode === 200) {
      ...
      try {
        const result = execSync(
          `docker exec shenmay-db psql --U shenmay -d shenmay_ai -c "SELECT plan, status, max_customers, max_messages_month, max_agents FROM subscriptions WHERE tenant_id = '${tenantId}';"`,
          { encoding: 'utf8' }
        );
        console.log(result);
